...
SELECT PCPRODUT.CODPROD 
    ...
    , CASE WHEN (PCFILIAL.USAWMS = 'S') AND (PCPRODUT.USAWMS = 'S')                         
     ...
     ,                                                            
       ((NVL(PCEST.QTVENDMES1, 0) + NVL(PCEST.QTVENDMES2, 0) + NVL(PCEST.QTVENDMES3, 0)) / :DIASUTEISTRIMESTRE) 
       QTGIRODIA                                                  
     ,                                                            
       ((NVL(PCEST.QTVENDMES1, 0) + NVL(PCEST.QTVENDMES2, 0) + NVL(PCEST.QTVENDMES3, 0)) / 3) 
       QTGIROMES                                                    
  ...
  , DECODE(PARAMFILIAL.OBTERCOMOVARCHAR2('USAPOLITICACOMERCIALPRODFILIAL'), 'P', NVL(PCPRODUT.CUSTOPROXIMACOMPRA, 0), 
NVL(PCEST.CUSTOPROXIMACOMPRA, 0)) CUSTOPROXIMACOMPRA                          
  , DECODE(PARAMFILIAL.OBTERCOMOVARCHAR2('USAPOLITICACOMERCIALPRODFILIAL'), 'P', NVL(PCPRODUT.CUSTOFORNEC, 0), 
NVL(PCEST.CUSTOFORNEC, 0)) CUSTOFORNEC                                               
     ...
     , (DECODE(NVL(PCEST.QTGIRODIA, 0), 0, 0,                                                                
       ((PKG_ESTOQUE.ESTOQUE_DISPONIVEL(PCEST.CODPROD, PCEST.CODFILIAL, 'C') - (DECODE(PCFILIAL.CONSIDERAESTPENDSUGCOMPRA, 'N', 
0, NVL(PCEST.QTPENDENTE,0)))  ) / DECODE(NVL(PCEST.QTGIRODIA,0),0,1,PCEST.QTGIRODIA)))) AS ESTDIAS 
...
(DECODE(NVL(PCFILIAL.CONSIDERAESTPENDSUGCOMPRA, 'N'), 'N', 0, NVL(PCEST.QTPENDENTE,0))) ))                                  AS 
SUGCOMPRA                                                             
