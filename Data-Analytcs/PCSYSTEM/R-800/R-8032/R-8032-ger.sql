...
--SEM MACROS
 ...
 SELECT 
    ...
    */
        ...
        B.meta_fim, 
        B.meta_pos, 
        B.perc_meta_fim, B.perc_meta_fim_fnc,
        B.perc_meta_pos, B.perc_meta_pos_fnc
...
(        select 
       ...
       :DATA_FIM DATA_FINAL,
       tab2.*,            
   fnc_wn_premiacao_comissao(:COD_FILIAL, tab2.cod_rca, to_date(:DATA_INI,'dd-mm-rrrr'), to_date(:DATA_FIM,'dd-mm-rrrr'), 4) perc_meta_fim_fnc,
   fnc_wn_premiacao_comissao(:COD_FILIAL, tab2.cod_rca, to_date(:DATA_INI,'dd-mm-rrrr'), to_date(:DATA_FIM,'dd-mm-rrrr'), 5) perc_meta_pos_fnc,
    ...
    from pcusuari us, 
     ...
     where 1=1 
       ...
       and vd.CODFILIAL in (2)
        and (
          case when :COD_GERENTE != '' then  sp.codgerente in (:COD_GERENTE) else  sp.codgerente in (select sp.codgerente from sp)
              when :COD_SUPERVISOR !='' then US.CODSUPERVISOR in (:COD_SUPERVISOR) else US.CODSUPERVISOR in (select US.CODSUPERVISOR from us)
          when :COD_RCA !='' then  US.CODUSUR in (:COD_RCA) else US.CODUSUR in (select US.CODUSUR from us)
        )
