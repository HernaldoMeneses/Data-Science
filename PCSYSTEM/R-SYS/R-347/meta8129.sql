...
round(p1."VEnda_Geral"/p2.dias_corridos*(p2.dias_corridos+p2.dias_restantes),2) as Projecao,
round(round(p1."VEnda_Geral"/p2.dias_corridos*(p2.dias_corridos+p2.dias_restantes),2)/ p1."META_Geral"*100,2) as Percentual_Projecao,  
  p1."BAse Clientes", p1."CLI. Posit.", p1."perc_Posit"
, p1."Perc_Venda_IND.", p1."Perc_Posit_IND", p1."META_LUCRO", p1."Perc_Lucrat",  
--p1."LIQUIDADO",  
p1.Devol, p1."Perc_DEVOL", p1."Perc_INADIMP"
 from (
...
select * 
from 
(select count(distinct(a.data)) as dias_corridos from pcdatas a where a.data >= to_date(:DTINICIO,'dd/mm/yyyy') and a.data < to_date(SYSDATE,'dd/mm/yyyy') )
, (select count(distinct(a.data)) as dias_restantes from pcdatas a where a.data > to_date(SYSDATE,'dd/mm/yyyy') and a.data <= to_date(:DTFIM,'dd/mm/yyyy') )
, (select count(distinct(a.data)) as dias_totais from pcdatas a where a.data >= to_date(:DTINICIO,'dd/mm/yyyy') and a.data <= to_date(:DTFIM,'dd/mm/yyyy') )
) p2
