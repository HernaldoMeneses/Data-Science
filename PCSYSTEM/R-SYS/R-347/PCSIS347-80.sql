----------------------------------
Timestamp: 09:00:15.243
SELECT LAST_DAY(:DTFIM) DATA FROM DUAL
DTFIM = '31/03/2024'
----------------------------------
Timestamp: 09:00:15.259
SELECT TRUNC(SYSDATE) DATA FROM DUAL
----------------------------------
Timestamp: 09:00:15.269
SELECT CODIGO,
       RAZAOSOCIAL,
       CGC
FROM PCFILIAL
WHERE CODIGO IN (SELECT PCLIB.CODIGOA FROM PCLIB WHERE PCLIB.CODFUNC = 1261 AND 
PCLIB.CODTABELA = 1) AND TO_CHAR(CODIGO) = :PARAM1
PARAM1 = '2'
----------------------------------
Timestamp: 09:00:15.305
SELECT NVL(PCFILIAL.USADIAUTILFILIAL,'N') USADIAUTILFILIAL
FROM PCFILIAL
WHERE PCFILIAL.CODIGO = 
'2'
----------------------------------
Timestamp: 09:00:15.328
SELECT CODIGO,
       RAZAOSOCIAL,
       CGC
FROM PCFILIAL
...
SELECT pcusuari.codusur, pcusuari.nome, pcusuari.dtinicio,
       ...
       NVL ((SELECT SUM (pcmetarca.vlvendaprev)
              ...
              WHERE pcmetarca.codusur = pcusuari.codusur
                ...
                AND pcmetarca.DATA BETWEEN :DTINICIO AND :DTFIM),
            0
           ) AS vlmeta,
       NVL ((SELECT SUM (pcmetarca.vlvendaprev)
               FROM pcmetarca
              WHERE pcmetarca.codusur = pcusuari.codusur
                ...
                AND pcmetarca.DATA BETWEEN TRUNC(:DTINICIO,'mm') AND 
LAST_DAY(:DTFIM)),            0
           ) AS vlmetatotal,
       NVL (vendas.vlvenda, 0) vlvenda, NVL( vendas.vltabela,0) vltabela,

       NVL (decode(NVL(vendas.VLVENDA1,0),0,0,
vendas.prazomedio/vendas.VLVENDA1), 0) prazomedio,       NVL (vendas.qtcli, 0) clqtcli, NVL(FINANC.VALOR, 0) CLVALOR,
...
WHERE ( (PCCLIENT.CODUSUR1 = PCUSUARI.CODUSUR)  OR ( (PCCLIENT.CODUSUR2 = 
PCUSUARI.CODUSUR ) AND ( NVL(PCCLIENT.codusur1,0) <> NVL(PCCLIENT.codusur2,0)) 
...
AND ( PCCLIENT.DTULTCOMP >= trunc(sysdate - (SELECT nvl(numdiascliinativ,0) 
FROM pcconsum) ) )  ) clqtcliativ, 
           (SELECT 

      SUM(ROUND(DECODE(PCPEDC.CONDVENDA,5,0,6,0,11,0,12,0,(NVL(PCPEDI.PVENDA, 
0) + NVL(PCPEDI.VLOUTRASDESP, 0) + NVL(PCPEDI.VLFRETE, 0)) * NVL(PCPEDI.QT, 0)),
2)) AS vlvendatotal                     FROM PCPEDI, 
                          PCPEDC, 
                          PCUSUARI A, 
                          PCPRODUT, 
                          PCFORNEC, 
...
'BNRP', 'CRED')     and PCUSUARI.CODSUPERVISOR = :CODSUPERVISOR
...
PARAM1 = '2'
----------------------------------
Timestamp: 09:00:17.214
SELECT NVL(PCFILIAL.USADIAUTILFILIAL,'N') USADIAUTILFILIAL
FROM PCFILIAL
WHERE PCFILIAL.CODIGO = 
'2'
----------------------------------
Timestamp: 09:00:17.244
SELECT CODIGO,
       RAZAOSOCIAL,
       CGC
FROM PCFILIAL
...
PARAM1 = '2'
----------------------------------
Timestamp: 09:00:19.771

SELECT NVL (decode(NVL(vendas.vlvenda,0),0,0,vendas.prazomedio/vendas.vlvenda), 
0) prazomedio,       NVL (vendas.qtcli, 0) clqtcli,  NVL (vendas.numitens, 0) numitens,
       NVL (MIX.QTMIX,0) clQtMix,
       (  (SELECT COUNT(DISTINCT(PCCLIENT.CODCLI)) 
     ...
     OR ( NVL(PCCLIENT.codusur2,0) = pcusuari.codusur  
         AND PCUSUARI.CODSUPERVISOR = :CODSUPERVISOR 
   )) 
        + (SELECT COUNT(DISTINCT(PCCLIENT.CODCLI)) 
             FROM pcusurcli, pcclient, pcusuari
            WHERE NVL(pcusurcli.codusur,0) = pcusuari.codusur
         AND pcusurcli.codusur <> NVL(PCCLIENT.CODUSUR1,0)  
         AND PCUSUARI.CODSUPERVISOR = :CODSUPERVISOR 
         AND pcusurcli.codusur <> NVL(PCCLIENT.CODUSUR2,0)  
         AND PCUSUARI.CODSUPERVISOR = :CODSUPERVISOR 
...
( SELECT COUNT(DISTINCT(PCCLIENT.CODCLI)) QTCLIATIVOS FROM PCCLIENT, pcusuari

WHERE PCUSUARI.CODSUPERVISOR = :CODSUPERVISOR  and ( (PCCLIENT.CODUSUR1 = 
PCUSUARI.CODUSUR)  OR ( (PCCLIENT.CODUSUR2 = PCUSUARI.CODUSUR ) AND ( 
NVL(PCCLIENT.codusur1,0) <> NVL(PCCLIENT.codusur2,0)) )  OR         EXISTS (SELECT PCUSURCLI.CODUSUR
                   FROM PCUSURCLI
                  WHERE PCUSURCLI.CODUSUR = PCUSUARI.CODUSUR
                    AND PCUSUARI.CODSUPERVISOR = :CODSUPERVISOR
                    AND PCUSURCLI.CODCLI  = PCCLIENT.CODCLI
...
FROM pcconsum) ) )  ) clqtcliativ 
 ...
 (SELECT   SUM(ROUND(DECODE(PCPEDC.CONDVENDA,5,0,6,0,11,0,12,0,
(NVL(PCPEDI.PVENDA, 0) + NVL(PCPEDI.VLOUTRASDESP, 0) + NVL(PCPEDI.VLFRETE, 0)) 
* NVL(PCPEDI.QT, 0)),2)) AS vlvenda,                  COUNT(*) QTNF,
                 NVL(SUM(PCPEDC.NUMITENS),0) NUMITENS,

                 SUM(NVL(PCPEDC.PRAZOMEDIO, 0) *  (NVL(PCPEDI.QT, 0) * 
(NVL(PCPEDI.PVENDA, 0) + NVL(PCPEDI.VLOUTRASDESP, 0) + NVL(PCPEDI.VLFRETE, 0))) 
) AS prazomedio,                 COUNT (DISTINCT (pcpedc.codcli)) qtcli
          FROM PCPEDI,
               PCPEDC,
               PCPRODUT,
               PCFORNEC,
         ...
         WHERE PCPEDI.NUMPED = PCPEDC.NUMPED
           ...
           AND PCUSUARI.CODSUPERVISOR = PCSUPERV.CODSUPERVISOR
           AND PCPEDC.CODPRACA = PCPRACA.CODPRACA
             AND PCPEDC.CONDVENDA  NOT IN (4,5,6,8,10,11,12,13, 20, 98)
             AND pcpedc.dtcancel IS NULL
 AND pcpedc.vlatend > 0 
             AND pcpedc.codfilial IN ('2')
             AND pcpedc.DATA BETWEEN :DTINICIO AND :DTFIM 
       ...
       (SELECT COUNT(DISTINCT(PCPEDI.CODPROD)) AS QTMIX
          FROM PCPEDI, PCUSUARI, PCSUPERV, PCPEDC
         WHERE PCPEDI.NUMPED = PCPEDC.NUMPED
           AND NVL(PCPEDI.BONIFIC, 'N') =  'N' 
           AND  PCPEDC.CODUSUR  = PCUSUARI.CODUSUR
