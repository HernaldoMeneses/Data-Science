...
--#--------------------------------------------------------------------Thanks.
select distinct(Gerentes.DTINICIO) as DTINICIO,
Gerentes.DTFIM,
pcgerente.codgerente,
pcgerente.nomegerente, 
Gerentes.codsupervisor, Gerentes.nomesup,

 sum(Gerentes.vlmetatotal) as  vlmetatotal,
  sum(Gerentes.vlvenda) as vlvenda,
   case when sum(Gerentes.vlmetatotal)>0 then round(sum(Gerentes.vlvenda)/sum(Gerentes.vlmetatotal)*100,2) else 0 end perc_meta_fin,


 sum(Gerentes.clqtcliativ) as clqtcliativ,
     sum(Gerentes.Positivacao) as Positivacao,
     case when sum(Gerentes.clqtcliativ)>0 then round( sum(Gerentes.Positivacao)/sum(Gerentes.clqtcliativ)*100,2) else 0 end Perc_Positva,

     
     avg(Gerentes.PERC_COB_FORN_FIN) as PERC_COB_FORN_FIN,
    avg(Gerentes.PERC_COB_FORN_POS) as PERC_COB_FORN_POS,
     
     sum(Gerentes.META_LUCRO) as META_LUCRO,

     sum(Gerentes.lucro) as LUCRO,
     avg(Gerentes.PERC_COB_LUCRO) as PERC_COB_LUCRO,
     
   sum(Gerentes.LIQUIDEZ) as LIQUIDEZ,
     
--     tabelao.per_lucro,
--     tabelao.venda,

    sum(Gerentes.Devol) as Devol,
     case when sum(Gerentes.vlvenda) > 0 then COALESCE( sum(Gerentes.Devol),0)/sum(Gerentes.vlvenda)*100 else 0 end Perc_Devol,
      avg(Gerentes.PERC_INAD) as PERC_INAD
