...
--PCFORNEC.codfornec,PCPRODUT.CODPROD, PCPEDC.CODCLI,
PCPEDC.CODUSUR,
...
nvl(pcpedi.vlfrete,0)), 
    ...
    0 
           END) - SUM(CASE                                                                                                                 
                    ...
                    0
                      END) VLVENDA,                                                                                                        
   round(SUM((PCPEDI.qt)*NVL(PCPEDI.vlcustofin,0)),2) AS VLCUSTOFIN,
   --round((PCPEDI.qt)*NVL(PCPEDI.vlcustofin,0),2) AS VLCUSTOFIN,
  --round(NVL(PCPEDI.vlcustofin,0),2) AS VLCUSTOFIN,
...
WHERE 1=1
--H.Meneses
AND PCPEDC.DATA >= to_date(:DTINICIO,'dd/mm/yyyy') AND PCPEDC.DATA <= to_date(:DTFIM,'dd/mm/yyyy')
AND PCPEDI.DATA >= to_date(:DTINICIO,'dd/mm/yyyy') AND PCPEDI.DATA <= to_date(:DTFIM,'dd/mm/yyyy')
...
GROUP BY  
--PCPEDC.CODSUPERVISOR , PCSUPERV.NOME, PCPEDC.POSICAO, PCPEDC.DATA, PCPEDC.CODUSUR, PCPEDC.CODFILIAL, PCPRODUT.CODEPTO, PCFORNEC.codfornec, PCPRODUT.CODPROD, PCPEDC.CODCLI, PCPEDC.CODGERENTE
PCPEDC.CODUSUR
